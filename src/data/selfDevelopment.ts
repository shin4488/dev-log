...
フロントエンドとバックエンドのインターフェースにはGraphQLを使用した。`,
  },
  {
    ...
    techAppeal: '企業ロゴ画像を日次バッチで取得・保存し表示速度を向上。',
...
としたことで、リアルタイムの処理負担が減り、投稿取得のサーバサイドの処理時間が1秒ほど短縮した。`,
  },
  {
    ...
    techAppeal: '三角関数やベクトルを使った精密な当たり判定を実装。',
...
「バットとボール」「ボールと盤」などの当たり判定には、三角関数やベクトルを利用してより厳密な当たり判定を行うようにした。`,
  },
  {
    ...
    overview: '試合日の順位表を自動投稿。フォローするだけで最新順位が届く。',
    techAppeal: 'フォローするだけで最新順位を受け取れる手軽さを実現。',
...
順位表を教えてくれるようになっており、比較的気軽に使える。`,
  },
  {
    ...
    techAppeal: '翻訳処理をGASで実装し、APIコストを抑えて運用。',
...
（GAS側でAPIのエンドポイントを公開できるため、自作したGASの翻訳用のAPIを叩いている。）`,
  },
  {
    ...
    overview: '1分間弾を避け続けるシンプルなゲーム。Renderで無料デプロイ。',
    techAppeal: 'Renderで無料ホスティングし手軽に遊べる。',
